...
<c:if test="${!(fn:contains(jspStoreImgDir, '://'))}">
	<c:set value="${pageContext.request.scheme}://${pageContext.request.serverName}${portUsed}" var="eHostPath" />
	...
	<c:if test="${not empty remoteConfig && not empty remoteConfig.webHostName }">
		<c:set var="eHostPath" value="${fn:toLowerCase(pageContext.request.scheme)}://${fn:toLowerCase(remoteConfig.webHostName)}"/>
	</c:if>
