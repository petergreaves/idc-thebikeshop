...
<head>
	...
	<title>
		<%--
		//  If the store is closed or suspended, we get the message state _ERR_BAD_STORE_STATE (CWNCS1072E).
	...
	<c:if test="${errorBean.messageKey eq '_ERR_CMD_CMD_NOT_FOUND' || errorBean.messageKey eq '_ERR_VIEW_NAME_NOT_FOUND'}">
		...
		<%-- 
			Ensure 'homePageUrl' SEO URL is not broken before sending to homepage. If SEO is broken then the URL will be bad and cause infinite loop.
		--%>
		<c:if test="${!fn:contains(homePageUrl, 'HomePageURLWithLang')}">
