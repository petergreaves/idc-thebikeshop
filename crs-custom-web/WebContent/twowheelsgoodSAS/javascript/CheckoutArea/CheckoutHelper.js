...
CheckoutHelperJS = {
    ...
    /**
     * This function updates the order displayed in Shipping information on the page.
     */
    ...
    canCheckoutContinue: function (userType) {
        ...
        if (userType != undefined && userType === "G") {
            var subscriptionAttributes = $('table[id="order_details"]')
                .find('input[id^="selectedAttr_"]')
                .filter(function (key, input) {
                    var value = (input.value + "").toLowerCase();
                    if (value) {
                        return value.indexOf("subscription") == 0;
                    }
                    else {
                        return false;
                    }
                });
            if (subscriptionAttributes.length > 0) {
                MessageHelper.displayErrorMessage(MessageHelper.messages["ERROR_GUEST_USER_SUBMIT_SUBSCRIPTION_ORDER"]);
                return false;
            }
        }
        return true;
